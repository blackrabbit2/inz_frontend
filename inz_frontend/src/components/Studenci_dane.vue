<template>
  <v-row justify="center" :key="studenci_view">
    <v-expansion-panels>
      <v-expansion-panel>
        <v-expansion-panel-header> Studenci dane </v-expansion-panel-header>
        <v-expansion-panel-content>
          <v-card v-if="this.studenci.length">
            <v-card-title>
              Studenci
              <v-spacer></v-spacer>
              <v-text-field
            ...
            <v-form v-model="valid">
              <v-container>
                <v-row justify="center">
                  ...
                  <v-col cols="16" md="4">
                    <v-btn depressed color="primary" @click="generuj_studentow">
                      Wygeneruj
                    </v-btn>
                  </v-col>
                </v-row>
...
export default {
  ...
  methods: {
    ...
    init_studenci: function () {
      this.$api.get("studenci/?grupa=" + this.grupa.id).then((response) => {
        response.data.map((x) => {
          this.studenci.push({
            id: x.id,

<template>
  <v-row justify="center">
    <v-expansion-panel v-for="grupa in grupy" :key="grupa.id">
      ...
      >
        <template v-slot:activator="{ on, attrs }">
          <v-expansion-panel-header v-bind="attrs" v-on="on">
            <v-row justify="space-between">
              <v-col cols="9" sm="5">
                {{
                  grupa.kod_grupy +
                  " " +
                  grupa.dzien_tygodnia +
                  ", godzina: " +
                  grupa.godzina
                }}
              </v-col>
        ...
        <v-card>
          <v-toolbar dark color="primary">
            <v-btn icon dark @click="grupa.dialog = false">
